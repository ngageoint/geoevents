...
links.Timeline.prototype.createEventBox = function(item) {
    ...
    if (typeof $!="undefined" && $().popover && item.popover){
        $(divBox).popover(item.popover);
    }
    if (typeof $!="undefined" && $().click && item.click){
        $(divBox).click(item.click);
    }
    if (item.create){
        item.create();
    }
...
links.Timeline.prototype.redrawCurrentTime = function() {
    ...
    var x = this.timeToScreen(nowOffset) || 0;
...
links.Timeline.prototype.redrawCustomTime = function() {
    ...
    var x = this.timeToScreen(this.customTime) || 0,
