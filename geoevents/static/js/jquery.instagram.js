...
(function ($){
    $.fn.instagram = function (options) {
        ...
        $.ajax({
            ...
            success: settings.successFunction ||function (res) {
                ...
                if (limit > 0) {
                    for (var i = 0; i < limit; i++) {
                        var el = createPhotoElement(res.data[i]);
                        if (settings.returnDivs){
                            returnedDivs.push(el);
                        } else {
                            that.append(el);
                        }
                    }
                ...
                settings.onComplete != null && typeof settings.onComplete == 'function' && settings.onComplete(res.data, res,returnedDivs);
            }
        });
