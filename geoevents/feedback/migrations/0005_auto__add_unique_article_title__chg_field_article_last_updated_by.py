...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_unique('feedback_article', ['title'])


        # Changing field 'Article.last_updated_by'
        db.alter_column('feedback_article', 'last_updated_by_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'], null=True))

    def backwards(self, orm):
        # Removing unique constraint on 'Article', fields ['title']
        db.delete_unique('feedback_article', ['title'])


        # User chose to not deal with backwards NULL issues for 'Article.last_updated_by'
        raise RuntimeError("Cannot reverse this migration. 'Article.last_updated_by' and its values cannot be restored.")
    ...
    models = {
        ...
        'feedback.article': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_updated_by': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']", 'null': 'True', 'blank': 'True'}),
